...
class ObstacleMap:
    def __init__(self, size: int = 100, resolution: float = 1.0, angle_range: tuple = (-90, 90)):
        """
        Initialize a 2D grid map for obstacle detection.
    ...
    def update_obstacle(self, distance: float, angle_deg: float) -> None:
        """
        Update the grid map with a new obstacle reading.
...
def print_map(obstacle_map: ObstacleMap):
    """
    Print the 2D grid visualization of the obstacle map.
